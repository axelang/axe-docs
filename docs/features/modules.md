...
Axe provides a comprehensive standard library organized by functionality:

### I/O Operations

```axe
use std.io (println, print_str, read_file, write_file);
...
```axe
use std.string (concat, substr, find_substr, str_len);

def process_text(input: string): string {
    val processed: string = concat(input, str("!"));
    return processed;
}
```
...
```axe
use std.lists (StringList, append, contains);

def manage_items(items: ref StringList) {
    StringList.push(items, arena, str("item1"));
    val has_item: bool = StringList.contains(items, str("item1"));
}
```
...
```axe
use std.os (get_cwd, get_executable_dir, exec);

def run_command() {
    val cwd: string = get_cwd();
    val exit_code: i32 = exec(str("ls -la"));
}
```
...
```axe
use std.net (http_get, http_post, HttpResponse);

def fetch_data(url: string): HttpResponse {
    return http_get(url);
}
```
...
def handle_json() {
    val data: string = str(r#"{"name":"test"}"#);
    // JSON operations
}
```
...
```axe
use std.crypto (sha256, base64_encode);
